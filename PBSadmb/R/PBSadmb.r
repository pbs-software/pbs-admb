...
#-----------------------------------------------RH
admb=function(prefix="",pkg="PBSadmb",wdf="admbWin.txt",optfile="ADopts.txt",hnam=NULL){
	if (!require(PBSmodelling)) stop("!!!!!Install package PBSmodelling!!!!!")
	assign(pkg,list(pkg=pkg,func="admb",useCols=NULL),envir=.GlobalEnv)
	...
	temp <- readLines(wnam)
	temp <- gsub("action=admbWin.txt",paste("action=\"editADfile(`",wtmp,"`)\"",sep=""),temp)
	if (!is.null(prefix) && prefix!="")
		temp <- gsub("value=vonb",paste("value=",prefix,sep=""),temp)
	temp <- gsub("C:/Utils/ADMB",adir,temp)
	temp <- gsub("ADopts.txt",optfile,temp)
	#---examples---
	...
	for (i in eprf) 
		enew=c(enew,paste("menuitem label=",i," function=doAction action=\"copyFiles(`",
			i,".`,dir0=`",edir,"`)\"",sep=""))
		#enew=c(enew,paste("menuitem label=",i," function=openExamples action=PBSadmb,",i,",.tpl,.pin,.dat,.r,.pdf",sep=""))
	temp <- gsub("nitems=0",paste("nitems=",length(eprf),sep=""),temp)
	temp <- gsub("menuitem label=findExamples",paste(enew,collapse="\n\t"),temp)
	if (!is.null(hnam) && is.character(hnam))
		temp <- gsub("#import=\"\"",paste("import=\"",hnam,"\"",sep=""),temp)
	writeLines(temp,con=wtmp)
	#editADfile(wtmp); browser(); return()
	#.onClose <<- function() { save("PBSadmb",file="PBSadmb.rda") }
	createWin(wtmp) 
	.win.initAD()
	invisible() }
#---------------------------------------------admb
